...
use fuzzburn::ir::program::AutogradProgram;
...
fuzz_target!(|prog: AutogradProgram| {
    ...
    if let Err(msg) = interpreter::run_autograd_program(&prog, &cfg) {
        let display = prog.ssa(&cfg);
        match cfg.mode {
            HarnessMode::PanicOnFirstError => panic!("fuzz_autograd CRASH:\n{display}\nerror: {msg}"),
